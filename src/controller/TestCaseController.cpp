...
void TestCaseController::runTestCaseCPSat(const HttpRequestPtr &req,
                                          function<void(const HttpResponsePtr &)> &&callback)
{
    auto start_time = high_resolution_clock::now();
    ...
    try
    {
        ...
        if (body.empty())
        {
            LOG_WARN << "[TestCaseCPSat] Empty body";
            json err;
            err["status"] = "error";
            err["message"] = "empty body";
            err["test_case_name"] = "";
        ...
        // Get time limit (optional, default 300 seconds)
        int time_limit = 300;
        ...
        jout["status"] = "success";
        jout["test_case_name"] = test_case_name;
        jout["method"] = "cpsat_all_constraints";
        ...
        jout["timing"]["total_time_seconds"] = total_duration / 1000.0;
        jout["timing"]["phase1_init_ms"] = init_duration;
        jout["timing"]["cpsat_solve_ms"] = cpsat_duration;
        jout["timing"]["time_limit_seconds"] = time_limit;
    ...
    catch (const json::parse_error &ex)
    {
        ...
        LOG_ERROR << "[TestCaseCPSat] JSON parse error: " << ex.what();
        json err;
        err["status"] = "error";
        err["message"] = "JSON parse error: " + string(ex.what());
        err["timing"] = json::object();
    ...
    catch (const exception &ex)
    {
        ...
        LOG_ERROR << "[TestCaseCPSat] Exception: " << ex.what();
        json err;
        err["status"] = "error";
        err["message"] = ex.what();
        err["timing"] = json::object();
